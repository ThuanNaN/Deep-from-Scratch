...
class Embedding(nn.Embedding, LoRALayer):
    def __init__(self, 
                 ...
                 r:int = 0, lora_alpha: int = 1, lora_dropout: float = 0., merge_weights: bool = True, 
                 **kwargs
                 ) -> None:
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.r > 0 and not self.merged:
            ...
            after_A = F.embedding(
                ...
                self.padding_idx, self.max_norm, self.norm_type, self.scale_grad_by_freqm, self.sparse
            )
            result += (after_A @ self.lora_B.transpose(0, 1))*self.scaling
            return result
        else:
            return nn.Embedding(self, x)
...
class MergeLinear(nn.Linear, LoRALayer):
    def __init__(self, in_features: int, out_features: int,
                 r: int = 0, lora_alpha: int = 1, lora_dropout: float = 0., enable_lora: List[bool] = [False],
        ...
        assert out_features % len(enable_lora) == 0, "The length of enable_lora must devide out_features"
    ...
    def merge_AB(self):
        ...
        delta_w = F.conv1d(self.lora_A.unsqueeze(0), self.lora_B.unsqueeze(-1), groups=sum(self.enable_lora)
                           ).squeeze(0)
...
class ConvLoRA(nn.Module, LoRALayer):
    def __init__(self, 
                 ...
                 r: int = 0, lora_alpha: int = 0, lora_dropout: float = 0., merge_weights: bool = True,
                 **kwargs) -> None:
...
if __name__ == "__main__":
    # model = nn.Linear(100, 10, bias=False) # 1000 params

    model = Linear(100, 10, r=3, bias=False)
